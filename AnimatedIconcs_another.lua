...
function defalut_config()
    ...
    savejson(convertTableToJsonString(config), "moonloader/AnimatedIconcs/Another/AnimatedIconcs.json")
end

if doesFileExist("moonloader/AnimatedIconcs/Another/AnimatedIconcs.json") then
    local f = io.open("moonloader/AnimatedIconcs/Another/AnimatedIconcs.json")
...
function main()
	...
	if doesFileExist("moonloader/AnimatedIconcs/Another/outline_anim.txd") then
		if mad.get_txd('txd_outline_anim') ~= nil then
			txd_outline_anim = mad.get_txd('txd_outline_anim')
		else
			txd_outline_anim = mad.load_txd(getWorkingDirectory() .. '//AnimatedIconcs//Another//outline_anim.txd', 'txd_outline_anim')
		end
	...
	---------------------txd_anim--------------------txd_anim----------
	local mask = getWorkingDirectory() .. "//AnimatedIconcs//Another//*.txd"
	...
	while handle and file do
		if file ~= "." and file ~= ".." and file ~= "outline_anim.txd" then
			...
			if doesFileExist("moonloader/AnimatedIconcs/Another/"..file) and checktable(id_gun, tdx_gsub) then
				if mad.get_txd(tdx_gsub) ~= nil then -- Проверяет наличие txd в памяти, костыль на случай перезапуска скрипта(ов)
					...
					txd_file = mad.load_txd(getWorkingDirectory() .. '//AnimatedIconcs//Another//'..file, tdx_gsub)
				end
	...
	files = {}
	local time = get_file_modify_time(string.format("%s/AnimatedIconcs/Another/AnimatedIconcs.json",getWorkingDirectory()))
	if time ~= nil then
	  files[string.format("%s/AnimatedIconcs/Another/AnimatedIconcs.json",getWorkingDirectory())] = time
	end
