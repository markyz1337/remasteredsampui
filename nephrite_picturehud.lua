...
function main()
    ...
    if not doesDirectoryExist("moonloader/picturehud/nephrite") then createDirectory("moonloader/picturehud/nephrite") end
    if doesFileExist('moonloader/config/picturehud/nephrite/picturehud.json') then
        local f = io.open('moonloader/config/picturehud/nephrite/picturehud.json')
        ini_png = decodeJson(f:read('*a'))
        f:close()
    else
        local f = io.open('moonloader/config/picturehud/nephrite/picturehud.json', 'w')
        f:write(encodeJson({}))
        f:close()
        ini_png = {}
    end
    sampRegisterChatCommand("pihud", function() menu = not menu end)
    local handleFile, nameFile = findFirstFile("moonloader/picturehud/nephrite/*.png")
    ...
    if handleFile then
        if not nameFile then
            ...
            png[nameFile] = renderLoadTextureFromFile("moonloader\\picturehud\\nephrite\\"..nameFile)
    ...
    if edit then
        ...
        ini_png[select][slider.v].posx,ini_png[select][slider.v].posy = int_posX, int_posY
            if isKeyDown(key.VK_LBUTTON) then
                ...
                savejson(ini_png,'moonloader/config/picturehud/nephrite/picturehud.json')
            end
    end
