...
function main()
    ...
    if not doesDirectoryExist("moonloader/picturehud/bzone") then createDirectory("moonloader/picturehud/bzone") end
    if doesFileExist('moonloader/config/picturehud/bzone/picturehud.json') then
        local f = io.open('moonloader/config/picturehud/bzone/picturehud.json')
        ini_png = decodeJson(f:read('*a'))
        f:close()
    else
        local f = io.open('moonloader/config/picturehud/bzone/picturehud.json', 'w')
        f:write(encodeJson({}))
        f:close()
        ini_png = {}
    end
    sampRegisterChatCommand("pihud", function() menu = not menu end)
    local handleFile, nameFile = findFirstFile("moonloader/picturehud/bzone/*.png")
    ...
    if handleFile then
        if not nameFile then
            ...
            png[nameFile] = renderLoadTextureFromFile("moonloader\\picturehud\\bzone\\"..nameFile)
    ...
    if edit then
        ...
        ini_png[select][slider.v].posx,ini_png[select][slider.v].posy = int_posX, int_posY
            if isKeyDown(key.VK_LBUTTON) then
                ...
                edit = false
                menu = true
                sampAddChatMessage("Ïîëîæåíèå ñîõðàíåíî.", -1)
                savejson(ini_png,'moonloader/config/picturehud/bzone/picturehud.json')
            end
    end
...
function imgui.OnDrawFrame()
    ...
    imgui.Begin(u8'##pic', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
        ...
        imgui.Text(u8'Êàðòèíêà '..tostring(select))
        ...
        imgui.SameLine()
        imgui.Text(u8"ID êàðòèíêè")
        imgui.SameLine()
            if imgui.Button(u8"Äîáàâèòü") then
        ...
        imgui.SameLine()
        imgui.Text((check.v and u8"Âûêëþ÷èòü" or u8"Âêëþ÷èòü")..u8" ðåíäåð êàðòèíêè")
        if check.v then
        ...
        imgui.SameLine()
        imgui.Text((check_player.v and u8"Îòêðåïèòü" or u8"Ïðèêðåïèòü")..u8" êàðòèíêó "..(check_player.v and u8"îò ïåðñîíàæà" or u8"ê ïåðñîíàæó"))
        if not check_player.v then
            if imgui.Button(u8"Èçìåíèòü ïîëîæåíèå êàðòèíêè") then
                menu = false
                sampAddChatMessage("Äëÿ ñîõðàíåíèÿ ïîëîæåíèÿ íàæìèòå {ff6666}ËÊÌ{ffffff}.", -1)  
        ...
        imgui.Text(u8"Ðåêîìåíäóåìàÿ äëÿ ýòîé êàðòèíêè: Øèðèíà - "..x_r..u8", Äëèíà - "..y_r)
        imgui.SameLine()
        if imgui.Button(u8"Óñòàíîâèòü") then
        ...
        imgui.Text(u8"Äëÿ èçìåíåíèÿ ïàðàìåòðîâ, âêëþ÷èòå ðåíäåð êàðòèíêè")
        end
        imgui.EndChild()
