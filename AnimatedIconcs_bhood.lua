...
function defalut_config()
    ...
    savejson(convertTableToJsonString(config), "moonloader/AnimatedIconcs/Bhood/AnimatedIconcs.json")
end

if doesFileExist("moonloader/AnimatedIconcs/Bhood/AnimatedIconcs.json") then
    local f = io.open("moonloader/AnimatedIconcs/Bhood/AnimatedIconcs.json")
...
function main()
	...
	if doesFileExist("moonloader/AnimatedIconcs/Bhood/outline_anim.txd") then
		if mad.get_txd('txd_outline_anim') ~= nil then
			txd_outline_anim = mad.get_txd('txd_outline_anim')
		else
			txd_outline_anim = mad.load_txd(getWorkingDirectory() .. '//AnimatedIconcs//Bhood//outline_anim.txd', 'txd_outline_anim')
		end
	...
	---------------------txd_anim--------------------txd_anim----------
	local mask = getWorkingDirectory() .. "//AnimatedIconcs//Bhood//*.txd"
	...
	while handle and file do
		if file ~= "." and file ~= ".." and file ~= "outline_anim.txd" then
			...
			if doesFileExist("moonloader/AnimatedIconcs/Bhood/"..file) and checktable(id_gun, tdx_gsub) then
				if mad.get_txd(tdx_gsub) ~= nil then -- Проверяет наличие txd в памяти, костыль на случай перезапуска скрипта(ов)
					...
					txd_file = mad.load_txd(getWorkingDirectory() .. '//AnimatedIconcs//Bhood//'..file, tdx_gsub)
				end
	...
	files = {}
	local time = get_file_modify_time(string.format("%s/AnimatedIconcs/Bhood/AnimatedIconcs.json",getWorkingDirectory()))
	if time ~= nil then
	  files[string.format("%s/AnimatedIconcs/Bhood/AnimatedIconcs.json",getWorkingDirectory())] = time
	end
