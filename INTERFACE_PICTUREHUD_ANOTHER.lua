...
function main()
    ...
    if not doesDirectoryExist("moonloader/picturehud/another") then createDirectory("moonloader/picturehud/another") end
    if doesFileExist('moonloader/config/picturehud/another/picturehud.json') then
        local f = io.open('moonloader/config/picturehud/another/picturehud.json')
        ini_png = decodeJson(f:read('*a'))
        f:close()
    else
        local f = io.open('moonloader/config/picturehud/another/picturehud.json', 'w')
        f:write(encodeJson({}))
        f:close()
        ini_png = {}
    end
    sampRegisterChatCommand("pihud", function() menu = not menu end)
    local handleFile, nameFile = findFirstFile("moonloader/picturehud/another/*.png")
    ...
    if handleFile then
        if not nameFile then
            ...
            png[nameFile] = renderLoadTextureFromFile("moonloader\\picturehud\\another\\"..nameFile)
    ...
    if edit then
        ...
        ini_png[select][slider.v].posx,ini_png[select][slider.v].posy = int_posX, int_posY
            if isKeyDown(key.VK_LBUTTON) then
                ...
                savejson(ini_png,'moonloader/config/picturehud/another/picturehud.json')
            end
    end
